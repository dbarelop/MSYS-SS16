...
int8_t blueOsWaitTimeOut(BlueOsSignal* signal, uint16_t time)
{
	int8_t retval = 0;
	...
	if(_taskDelays[_currentTaskId] != TIMER_TIMEOUT_INACTIVE) //signal case
	{
		_taskDelays[_currentTaskId] = TIMER_TIMEOUT_INACTIVE;
	}
	else //timeout case
	{
		if(signal->_firstWaitingTask == _currentTask)
		{
			signal->_firstWaitingTask = signal->_firstWaitingTask->_nextWaitingTask;
		}
		else
		{
			tmp = signal->_firstWaitingTask;
			while(tmp != 0 && tmp->_nextWaitingTask != _currentTask)
			{
				tmp = tmp->_nextWaitingTask;
			}
			if(tmp)
			{
				tmp->_nextWaitingTask = tmp->_nextWaitingTask->_nextWaitingTask;
			}
		}
		retval = 1;
	}
